    ...
    try:
        ...
        with db_connect(news_db) as conn:
            ...
            if 'slug' not in columns:
                ...
                print("Migrating news database to add slug column...")

            if 'status' not in columns and 'slug' in columns:
                print("Adding status column to news_articles table...")
                cursor.execute('ALTER TABLE news_articles ADD COLUMN status TEXT DEFAULT "published"')
                needs_migration = True

            # Add email_sent column if it doesn't exist
            if 'email_sent' not in columns:
                print("Adding email_sent column to news_articles table...")
                cursor.execute('ALTER TABLE news_articles ADD COLUMN email_sent BOOLEAN DEFAULT 0')
                needs_migration = True

            if needs_migration and 'slug' not in columns:
                cursor.execute('DROP TABLE IF EXISTS news_articles')
                print("Recreating news database with new schema...")
    ...
    try:
        with db_connect(news_db) as conn:
            ...
            cursor.execute('''
                ...
                SET title = ?, slug = ?, content = ?, image_url = ?, status = ?, updated_at = CURRENT_TIMESTAMP
                WHERE id = ?
            ''', (title.strip(), slug, content.strip(), image_url, status, article_id))
            conn.commit()
...
def create_article():
    ...
    try:
        ...
        article_id, slug = create_article_db(title, content, image_url, status)
...
def update_article(article_id):
    ...
    try:
        ...
        success = update_article_db(article_id, title, content, image_url, status)
